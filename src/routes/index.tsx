...
function Index() {
  return (
    <main className="relative min-h-screen text-foreground overflow-hidden">
      ...
      />
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-md flex-col justify-start pb-[4%] px-6 pt-12 md:pt-12">
        {/* Logo */}
        <div className="mx-auto mb-auto pt-8 w-full max-w-[192px]">
        ...
        <nav className="flex flex-col gap-3" aria-label="Links">
          {links.map(({ label, sublabel, href, icon: Icon, accent }) => (
            <a
              ...
              className="group relative flex items-center gap-4 rounded-xl border border-white/10 bg-black/80 px-5 py-4 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] backdrop-blur-sm"
              style={{ 
                '--neon-color': accent,
